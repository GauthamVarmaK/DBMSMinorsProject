...
stock_df = pd.DataFrame(stock, columns=["comp_id", "in_stock", "lead_time", "normally_stocking"])
...
cursor.execute("SELECT * FROM price_slabs WHERE price_slabs.price > (SELECT AVG(price) FROM price_slabs);")
...
cursor.execute("SELECT location, count(*) as num FROM manufacturer GROUP BY(location) Having count(*)>1;")
...
cursor.execute("SELECT users.user_id, users.name, users.phone_number, orders.status FROM users LEFT JOIN orders ON users.user_id=orders.user_id and orders.status='Missing';")
components = cursor.fetchall()

components_df = pd.DataFrame(components, columns=["user)id", "name", "phone_number", "status"])
...
cursor.execute("SELECT users.user_id, orders.order_id, payments.trnx_no, payments.amount FROM users INNER JOIN orders ON users.user_id=orders.user_id INNER JOIN payments ON payments.order_id=orders.order_id where payments.amount>20000;")
payments = cursor.fetchall()

payments_df = pd.DataFrame(payments, columns=["user_id", "order_id", "trnx_no", "amount"])
