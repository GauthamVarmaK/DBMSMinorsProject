...
st.set_page_config(page_title="Mrinal Queries", layout="wide", page_icon="🐻")
...
FROM components 
WHERE category='LEDS' AND lifecycle='InProduction';
```
"""
)
cursor.execute(
...
SELECT comp_id 
FROM price_slabs 
WHERE price=(SELECT MIN(price) FROM price_slabs);
```
"""
)
cursor.execute(
...
```sql
SELECT users.name,payments.method
FROM users,orders,payments 
WHERE users.user_since = (SELECT max(user_since) FROM users) AND users.user_ID = orders.user_ID AND orders.order_ID = payments.order_ID AND orders.status = 'tobeshipped';
```
...
```sql
SELECT users.user_ID,users.name,payments.method
FROM users,orders,payments
WHERE user_since=(SELECT MIN(user_since) FROM users) AND users.user_ID=orders.user_ID AND orders.order_ID=payments.order_ID AND payments.method=(SELECT payments.method FROM payments WHERE payments.order_ID=orders.order_ID GROUP BY payments.method LIMIT 1);
```
"""
)
...
```sql
SELECT users.name 
FROM users,components,orders,order_comp 
WHERE users.user_ID=orders.user_ID AND orders.order_ID=order_comp.order_ID AND order_comp.comp_ID=components.comp_ID AND components.mount_type="Surface Mount";
```
"""
)
cursor.execute(
